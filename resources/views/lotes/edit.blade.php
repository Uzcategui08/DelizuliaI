...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        ...
        <a href="{{ route('lotes.show', $lote) }}" class="btn btn-outline-secondary">Volver</a>
    </div>
@stop
...
@section('content')
    ...
    <div class="alert alert-info">
        Puedes cambiar las cantidades del Día 1 y agregar/quitar productos.
        Los días seleccionados solo se <strong>agregan</strong> (no se eliminan) para evitar perder historial de merma.
    </div>
        ...
        <div class="card">
            <div class="card-body">
                <div class="form-row">
                    <div class="form-group col-md-6">
                        ...
                        <input type="text" class="form-control" id="nombre" name="nombre"
                               value="{{ old('nombre', $lote->nombre) }}" required>
                    </div>
                    <div class="form-group col-md-6">
                        <label for="fecha_inicio">Fecha inicio (opcional)</label>
                        <input type="date" class="form-control" id="fecha_inicio" name="fecha_inicio"
                               value="{{ old('fecha_inicio', optional($lote->fecha_inicio)->format('Y-m-d')) }}">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    @php
                        $oldDays = collect(old('dias', $selectedDays ?? []))->map(fn($d) => (int)$d);
                    @endphp
                    <select id="dias" name="dias[]" class="form-control" multiple>
                        @for($d = 2; $d <= 60; $d++)
                            <option value="{{ $d }}" @if($oldDays->contains($d)) selected @endif>
                                Día {{ $d }}
                            </option>
                        @endfor
                    </select>
                    <small class="text-muted">Puedes seleccionar días adicionales; los ya existentes se mantendrán.</small>
                </div>
...
<script>
    ...
    const initialProducts = @json(old('productos') ?: ($initialProducts ?? []));
